    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Gerador e checker de nitro by barbosa")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Gerador e checker de nitro by barbosa\a',
        ...
        self.slowType("Made by: barbosa & sny", .02)
        ...
        if USE_WEBHOOK:
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Quando um codigo valido for encontrado vai aparecer aqui para ser resgatado ( ass barbosa <3 ) ```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Deck Community Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido - by barbosa ")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Deck Community Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido - by barbosa\a', end='', flush=True)
